...
두 산출물은 **각자 빌드·반입**합니다. AeroOne ZIP 은 vendored open-notebook 트리, GJC workflow state, QA artifact/scratch 파일을 포함하지 않습니다(`offline_package.bat` 의 `/XD .gjc artifacts vendor` 및 `/XF .ug-*` 가산; §6 보호 제외목록은 제거하지 않고 추가만).
