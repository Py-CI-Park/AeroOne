...
- 기준 commit: `1.6.2` 릴리즈 예정 (`1.6.1 폐쇄망 smoke 결함 보강`)
- 갱신일: 2026-06-18
- 테스트 상태: backend `pytest tests` **175 passed** (실패 0), frontend Vitest **193 passed** (47 파일), `tsc --noEmit` / `next build` 성공
...
**가능합니다.** 단일 PC(`Mode A`) 와 LAN 다중 PC(`Mode B`) 운영 모두 유지되며, 1.6.2 기준 backend 175개·frontend 193개 회귀와 폐쇄망 smoke(문서 뷰어, AeroAI, run_all/Open Notebook readiness) 로 검증된 상태입니다.
...
- frontend 전체: **193 passed / 47 files**
...
기대 출력 예: `175 passed in <시간>`. 실패가 1건이라도 나오면 §15의 단계 보고서와 [`docs/reports/INDEX.md`](reports/INDEX.md) 를 거꾸로 읽어 어느 단계의 회귀인지 진단합니다.
...
| **backend 합계** | **175** | `pytest tests` 전체 통과 |
| **frontend 합계** | **193 / 47 files** | `npm test` 전체 통과 |
...
두 산출물은 **각자 빌드·반입**합니다. AeroOne ZIP 은 vendored open-notebook 트리, GJC workflow state, QA artifact/scratch 파일을 포함하지 않습니다(`offline_package.bat` 의 `/XD .gjc artifacts vendor` 및 `/XF .ug-*` 가산; `node_modules` 등 §6 보호 제외목록은 제거하지 않고 보존).
