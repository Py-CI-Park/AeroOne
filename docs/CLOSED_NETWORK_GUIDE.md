...
**가능합니다.** 단일 PC(`Mode A`) 와 LAN 다중 PC(`Mode B`) 운영 모두 유지되며, 1.12.2 기준 대시보드 한국 시간 표시·버전 날짜 모달 전용 표시·중앙 로그인 카드·헤더 아이디 단독 표시, 1.12.1 의 로그아웃 기록·선택 프로필(`display_name`)·사용자별 권한 수정 UX, 1.12.0 의 관리자 콘솔 권한 이해 카탈로그·감사 로그 전용 탭·세션 자동 새로고침, 1.11.0 의 관리자 로그인/CRUD same-origin 프록시·탭형 관리자 콘솔·RBAC/ResourceGrant 입력 UX, 1.10.0 의 NSA 서버측 접근제어까지 유지됩니다.

---

## 3. 현재 상태 스냅샷
> **릴리즈 경계:** 이 가이드의 배포 기준선은 패키지 검증된 `1.12.2`입니다. 아래 §9.3의 Office Studio 지침은 `feature/dashboard-enhancements` 브랜치의 **미릴리즈** 구현 안내이며, main 병합과 새 오프라인 패키지 검증 전에는 `1.12.2` 배포 기능으로 취급하거나 운영 배포에 포함하면 안 됩니다.

### 3.1 최근 릴리즈/단계 스냅샷

| 커밋 | 단계 | 의미 |
|---|---|---|
| `1.12.2` | patch | 헤더 버전 날짜 즉시 노출 제거(클릭한 업데이트 내역 모달에서만 날짜 표시), 대시보드 한국 시간 실시간 표시, 로그인 중앙 카드 UI, 로그인 후 헤더 아이디 단독 표시, 세션 탭 세로 가독성 보강 |
...
|| `1.10.0` | 단계 23 | RBAC 읽기 권한 상승 차단, `can_read_collection` 단일 정책, NSA 기본 접근 코드 제거와 서버측 권한/ResourceGrant 적용, 사용자별 메뉴 힌트, 자산/config-health 진단, 사용자·그룹·리소스 권한/RBAC 매트릭스, 접속자·세션 대시보드 |
...
### 3.4 릴리즈 1.12.2 폐쇄망 반입물

| 반입물 | 릴리즈/생성 위치 | 폐쇄망 배치 | 필수 여부 |
|---|---|---|---|
| `AeroOne-offline-1.12.2-YYYYMMDD-HHMMSS.zip` + `.sha256` | AeroOne GitHub Release `1.12.2` asset 또는 온라인 PC `dist\` | `D:\AeroOne\` 로 압축 해제 후 단일 PC는 `setup_offline.bat --local`, LAN은 옵션 없는 기본 실행 또는 `--allow-host=<IP>` | 필수 |
...
D:\AeroOne-bundle\
```
두 폴더를 형제로 두면 `D:\AeroOne\scripts\run_all.bat` 가 기본값(`..\AeroOne-bundle`)만으로 AeroOne 과 Open Notebook 을 순서대로 띄웁니다. 단일 PC 검증은 `scripts\run_all.bat --local`, LAN 운영은 옵션 없는 기본 실행(LAN 기본값) 또는 `--allow-host=<IP>` 를 사용합니다.
...
`closed_network` 모드는 단계 6에서 신설되었으며, HTTP 폐쇄망에서 쿠키가 살아 있는 상태에서 `JWT_SECRET_KEY` / `ADMIN_PASSWORD` 가 기본 sentinel이거나 길이가 부족할 때 부팅 즉시 거부합니다.
...
offline_package.bat      ─┘──→ ZIP 복사 ──→  압축 해제
                                              setup_offline.bat --local (단일 PC) / setup_offline.bat [--allow-host=<host>] (LAN, 옵션 없으면 LAN 기본)
                                              start_offline.bat --local (단일 PC) / start_offline.bat [--allow-host=<host>] (LAN, 옵션 없으면 LAN 기본)
                                              (관리자에서 Import / Sync)
```
...
7. 단일 PC는 `setup_offline.bat --local` — 사전 점검 통과 후 자동 설치. LAN은 옵션 없는 `setup_offline.bat`(LAN 기본) 또는 `--allow-host=<host>`
8. 단일 PC는 `start_offline.bat --local` — 두 포트 준비 시 브라우저 자동 오픈. LAN은 옵션 없는 `start_offline.bat`(LAN 기본) 또는 `--allow-host=<host>`
...
- [ ] **단계 7** — 설치. **`--dry-run` 옵션은 붙이지 마세요** (그 옵션은 실제 설치 없이 단계만 미리보기). 곧장 실제 설치를 시작하려면:
  ...
  - 미리 단계만 보고 싶으면 단일 PC는 `setup_offline.bat --dry-run --local` — 설치 안 함, 단계 흐름만 출력. LAN dry-run은 `--allow-host=<IP>`를 붙이거나 옵션 없는 실행(LAN 기본)을 사용한다. 실제 LAN 설치도 옵션 없는 실행은 LAN 기본임을 확인한 뒤에만 실행한다.
...
:: 형태 2 — 환경 변수 (자동화 스크립트 권장; 아래 옵션 없는 명령은 이 LAN 설정으로 실행)
...
| 검증 | 결과 | 발견 사항 |
|---|---|---|
| `setup_offline.bat --dry-run --local` | PASS | 단일 PC loopback 분기 6단계가 의도대로 |
...
| 시점 | 명령 / 절차 |
|---|---|
| PC 부팅 후 | 단일 PC는 `start_offline.bat --local`. LAN은 옵션 없는 `start_offline.bat`(LAN 기본) 또는 `--allow-host=<host>`/`AEROONE_ALLOW_HOST`를 유지 |
...
| 비밀번호 교체 | `setup_offline.bat` 이 설치마다 생성한 `backend\.env` 의 `ADMIN_PASSWORD` 로 최초 로그인한 뒤 `/admin` 콘솔의 **관리자 계정 / 비밀번호** 에서 직접 변경(변경 시 다른 세션 로그아웃). 재설치가 필요하면 단일 PC는 `setup_offline.bat --local`, 옵션 없는 명령은 LAN 기본으로만 사용한다. 공유 고정 초기 비밀번호는 사용하거나 문서에 기록하지 않으며, `.env.bak` 비밀 처리와 키 회전 영향은 §10.2를 따른다. |
...
1.10.0 부터 `/nsa` 의 기존 기본 접근 코드 가림막은 제거되었습니다. NSA 는 암호화 비밀 저장소가 아니라 `_database\nsa\` HTML 컬렉션이며, 서버는 관리자이거나 로그인 세션에 전역 `collections.nsa.read`/legacy `search.nsa.read` 권한 또는 일치하는 `collection:nsa` ResourceGrant 중 하나가 있을 때 목록·본문·검색을 제공합니다. 운영자는 관리자 화면에서 계정/그룹 권한 또는 리소스 grant 를 명시적으로 부여하고, 미부여 사용자가 403 을 받는지 확인해야 합니다.

### 9.3 Office Studio 권한·산출물 보존 (미릴리즈 feature-branch 안내)

> 이 절은 `feature/dashboard-enhancements`의 Office Studio 구현에만 적용합니다. `1.12.2` 릴리즈/패키지에는 포함되지 않으며, main 병합과 새 오프라인 패키지 검증 후에만 운영 지침으로 승격합니다.

- `/office-tools`와 `/api/v1/office-tools/*`는 `office.use` 권한이 필요하며, 설치 기본 관리자에는 부여된다. 일반 사용자에게 카드를 공개할 때는 카드 `visibility`뿐 아니라 이 권한도 명시적으로 부여한다.
- 생성·inspect·삭제·purge 요청은 CSRF를 검증한다. 타 사용자 job 조회·다운로드·삭제는 403이다.
- 산출물은 `backend\data\office_jobs\`에 저장되며 기본 보존 30일, 사용자당 100개/1GiB, 저장 볼륨 최소 여유 512MiB를 적용한다. 운영 환경에서는 `OFFICE_JOB_RETENTION_DAYS`, `OFFICE_JOB_MAX_JOBS_PER_OWNER`, `OFFICE_JOB_MAX_BYTES_PER_OWNER`, `OFFICE_JOB_MIN_FREE_DISK_BYTES`로 조정한다.
- 관리자는 `admin.office.manage` 권한과 CSRF로 `POST /api/v1/office-tools/jobs/admin/purge`를 호출해 만료 job을 purge한다. 같은 권한으로 `/api/v1/office-tools/jobs/admin/pending-receipts`에서 비활성·삭제 actor의 미해결 receipt를 안전하게 조회하고, CSRF를 포함한 개별 replay로 원 provenance를 유지한 복구를 수행한다. purge와 operator replay는 `admin_audit_events`에 별도 intent/결과를 기록한다.
...
| `backend\data\office_jobs\` | 사용자 생성 Office 산출물. 기본 30일 보존의 재생성 가능한 작업 캐시 | 업무상 재사용이 필요한 경우에만 별도 백업 |
...
1. 기존 설치를 덮어쓰기 전, 현재 `backend\.env` 전체를 앱 트리·일반 백업 ZIP 밖의 접근 제어된 비밀 보관소에 복사합니다. 이 파일의 `JWT_SECRET_KEY`는 로그인 세션과 LLM 연결 암호화의 루트이므로 `backend\data` 백업만으로는 대체할 수 없습니다.
2. DB·`storage`·`_database`를 복원한 뒤 단일 PC에서는 `setup_offline.bat --local`로 필요한 의존성/마이그레이션을 완료합니다. LAN은 옵션 없는 `setup_offline.bat`가 LAN 기본이므로 의도한 LAN 배포일 때만 사용합니다. setup이 새 `.env`를 만들었으면 보호한 기존 `backend\.env`를 되돌리고, 새 경로가 다르면 경로 설정만 조정하되 기존 `JWT_SECRET_KEY`를 보존합니다.
3. 기존 `.env`를 보존할 수 없거나 키 회전을 의도했다면, 새 `JWT_SECRET_KEY`로 **모든 기존 세션이 무효화됨**을 운영자에게 알리고 재로그인을 요구합니다. 이전 키로 암호화된 LLM 연결 키도 복호화할 수 없으므로 각 연결을 실제 비밀로 다시 등록하고 기본 연결을 다시 지정한 뒤 `/v1/models` 검증을 완료해야 합니다.
4. `setup_offline.bat`가 만든 `backend\.env.bak`에는 이전 JWT 키와 관리자 비밀번호가 평문으로 남습니다. 이 파일은 접근 제어하고 일반 백업·ZIP·티켓에 포함하지 말며, 복원/검증이 끝나 더는 필요 없으면 조직의 비밀 폐기 정책에 따라 안전하게 제거합니다.

`ensure_db_state.py`는 기존 DB의 alembic 메타 부재를 감지해 `alembic stamp head` 분기로 데이터 보존하며 진행합니다 (§11.3 참고).
...
`setup_offline.bat` 매 실행 시 PowerShell `RandomNumberGenerator` 로 다음 두 값을 새로 생성합니다. 따라서 복원 시 기존 `backend\.env`를 보존하지 않으면 세션이 무효화되고 LLM 연결 등록이 필요합니다(§10.2).

- `JWT_SECRET_KEY` — 64자 hex (32바이트)
- `ADMIN_PASSWORD` — 48자 hex (24바이트)

기존 `backend\.env`는 `backend\.env.bak`으로 자동 백업되며, 두 파일 모두 비밀 파일로 취급합니다. `.env.bak`을 패키지·일반 백업·문서에 포함하지 말고, 필요가 끝나면 안전하게 폐기합니다.

### 11.2 `validate_runtime_security` 분기

|| 모드 | 기본 sentinel 또는 짧은 secret 거부 |
|---|---|
| `development` | 면제 |
| `test` | 면제 |
...
| `start_offline.bat` 가 브라우저를 안 엶 | frontend 빌드 미완료 / `.next` 누락 | 단일 PC는 `setup_offline.bat --local` 재실행(`npm run build` 까지). 옵션 없는 `setup_offline.bat`은 LAN 기본이므로 LAN 재설치에만 사용 |
...
**Q. 동일 PC 에 이미 설치되어 있는데 다시 `setup_offline.bat --local` 을 돌려도 되는가?**
A. 가능합니다. 단, `JWT_SECRET_KEY`와 `ADMIN_PASSWORD`가 새 랜덤 값으로 재생성됩니다. 기존 키를 보존하려면 §10.2처럼 보호한 `backend\.env`를 복원하고, 회전한다면 기존 세션 무효화·LLM 연결 재등록·`.env.bak` 비밀 폐기 절차를 완료해야 합니다. LAN 재설치에서만 옵션 없는 `setup_offline.bat`(LAN 기본)을 사용합니다.
...
> 아래 파일별 건수는 1.8.0 시점 스냅샷이라 합계 행이 최신 전체 수와 다를 수 있습니다. 현재 권위 있는 전체 수는 backend **268** / frontend **313 (66 files)** 이며 아래 합계 행에도 이를 반영했습니다.
...
| **backend 합계 (현재)** | **268** | `pytest tests` 전체 통과 |
...
- [`docs/runbook/leantime-codeploy.md`](runbook/leantime-codeploy.md) — [미릴리즈 feature branch] Leantime 동거 런북(패키징 SHA-256 매니페스트·검증·백업/복구/롤백/방화벽 계약 포함)
- [`docs/runbook/leantime-oidc-ldap.md`](runbook/leantime-oidc-ldap.md) — [미릴리즈 feature branch] Leantime OIDC/LDAP 별도 세션 통합 절차(선택, AeroOne 쿠키 공유 금지)
...
대시보드에 NSA 카드가 추가되었습니다. 1.10.0 부터 `/nsa` 는 비밀번호 기반 기본 가림막이 아니라 서버측 접근제어를 사용합니다.
...
```cmd
:: 두 번들이 나란히 있을 때 — 아래 옵션 없는 run_all 명령은 LAN 기본
scripts\run_all.bat
scripts\run_all.bat --on-bundle D:\AeroOne-bundle    :: 번들 경로 지정, LAN 기본
scripts\run_all.bat --dry-run                         :: 5포트 preflight + 기동 계획만, LAN 기본
