...
|---|---|---|---|
| `AeroOne-offline-1.12.2-YYYYMMDD-HHMMSS.zip` + `.sha256` | AeroOne GitHub Release `1.12.2` asset 또는 온라인 PC `dist\` | `D:\AeroOne\` 로 압축 해제 후 `setup_offline.bat` | 필수 |
...
노출이 의심되면 서비스를 중지하고 [`scripts\rotate_aeroone_credentials.ps1`](../scripts/rotate_aeroone_credentials.ps1)을 사용합니다. 이 도구는 configured admin과 활성 admin을 prepare/commit 양쪽에서 검증하고, 전체 사용자 비밀번호와 세션을 SQLite 단일 트랜잭션으로 회전합니다. 비활성 사용자는 활성화하지 않으며 로그인 실패는 계정 상태를 숨기는 기존 정책대로 **401**입니다.
...
| `backend/tests/{unit,integration}/test_credential_rotation*.py` | 56 | 전체 자격·세션 회전, production provenance·ACL·hardlink, WAL recovery, ledger/journal, crash 재개, 복원 뒤 archive→신규 회전, old 401/new 200 |
...
- `scripts/rotate_aeroone_credentials.ps1`, `scripts/credential_rotation/*` — production 경계, DPAPI journal/recovery/bundle, 중단 재개, DB 복원 뒤 history archive 오케스트레이션
