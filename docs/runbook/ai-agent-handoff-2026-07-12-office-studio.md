...
- **표현 고급화**: 차트=검증 팔레트+라운드 막대(단일 막대 그라디언트)·부드러운 선+영역 그라디언트·파이 보더·툴팁/레전드(`echarts-beautify.ts`), 다이어그램=Mermaid 브랜드 컬러 `base` 테마+곡선 엣지.
- **입력 방식**: 차트·보고서는 **파일 업로드 ↔ 직접 입력(텍스트)** 토글(`DataInput`). 예제 선택 시 텍스트로 채워 형식을 눈으로 확인.
- **단계형 UX**: 3폼 모두 `StepSection` 으로 ① 입력 → ② 옵션/메타 → ③ 생성 번호 단계.
- **예제 샘플 13종**: 다이어그램 5(플로우·시퀀스·상태·간트·**로드맵**), 차트 5(막대·선·파이·산점·히스토그램), 보고서 3(매출·장애 사후분석·주간). 도구별 '예제' 칩(`SamplePicker`)으로 원클릭 로드. + 목적/설명란 **예시 프롬프트 칩**.
