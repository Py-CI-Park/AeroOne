...
- 임의 bundle 경로를 받지 않으며 `%USERPROFILE%\AeroOne-secure\incident-20260710\1.12.3-credentials.dpapi` 한 자리만 읽습니다.
...
- clipboard가 여전히 방금 복사한 비밀번호일 때만 30초 후 삭제합니다. 그 뒤 운영자가 복사한 다른 내용은 지우지 않습니다.
...
완료된 회전의 DB를 `recovery\aeroone-db-before-rotation.dpapi`와 논리적으로 같은 독립 일반 백업 상태로 복원한 뒤에는 old completed bundle을 재사용할 수 없습니다. SQLite의 비논리적 header change counter는 canonicalize하지만 schema·테이블·행·DB ID·integrity는 exact 비교합니다. 다음 순서를 지킵니다.
...
- 1.13.0-dev: backend full 347 passed, credential focused 79 passed, frontend 313 passed / 66 files
