...
- **P4**: (부분 구현) **일정 ✅** — 사용자별 이벤트 CRUD + 기간 겹침 조회(이벤트 모델 + 마이그레이션 `20260719_0021` + `ScheduleService` + REST + `SchedulePanel` 아젠다 UI, 소유자 스코프·CSRF, 단위 5 + 통합 5). **홈 브리핑 ✅** — 홈에서 다가오는 7일 일정 + 지식폴더 색인 요약을 실데이터로 조합(`HomeBriefing`, 카드 클릭→해당 탭). **알림·실행기록 뷰는 미착수**.
