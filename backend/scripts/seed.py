...
Seed markdown document.', encoding='utf-8')
            ...
            with database.session() as session:
                ...
                if admin is None:
                    admin = user_repo.create(username=settings.admin_username, password_hash=hash_password(settings.admin_password))

                category = session.query(Category).filter(Category.slug == 'briefing').one_or_none()
                if category is None:
                    category = Category(name='브리핑', slug='briefing', description='기본 브리핑 카테고리')
                    session.add(category)
                    session.flush()

                tag = session.query(Tag).filter(Tag.slug == 'aerospace').one_or_none()
                if tag is None:
                    tag = Tag(name='항공우주', slug='aerospace')
                    session.add(tag)
                    session.flush()

                newsletter = session.query(Newsletter).filter(Newsletter.source_identifier == 'markdown-sample-welcome').one_or_none()
                if newsletter is None:
                    ...
                    newsletter.assets.append(NewsletterAsset(asset_type=AssetType.MARKDOWN, file_path='markdown/newsletters/sample-welcome.md', is_primary=True))
