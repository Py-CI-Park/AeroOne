...
class LlmConnection(Base):
    """OpenAI 호환 LLM 엔드포인트 1건의 등록 정보(base_url + 암호화된 api_key).

    관리자가 UI 로 등록하고, ``api_key_encrypted`` 는 ``llm_crypto`` 대칭 암호화로만
    저장한다(평문 저장·응답 반환 금지). 활성 기본 연결은 최대 1개이며 서비스 계층의
    ``set_default`` 가 유일성을 보장한다.

    시각 컬럼은 AiConversation 선례대로 DB-side ``func.now()`` 로만 채운다(SQLite 는
