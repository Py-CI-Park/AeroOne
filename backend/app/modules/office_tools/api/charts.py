...
규칙 기반 폴백으로 내려간다(경고 첨부). 상위 라우터가 세션 로그인을 강제하므로 미로그인은
401 이다. 미지원 확장자/크기·행수 초과/잘못된 수동 스펙은 422 로 거부한다.
"""
...
@router.post('/inspect', response_model=ChartInspectResponse)
def inspect(
    data_file: UploadFile = File(...),
    settings: Settings = Depends(get_settings),
    user: User = Depends(get_current_user),
) -> ChartInspectResponse:
    """데이터 파일의 프로필(행/열/샘플)을 반환한다. 잘못된 입력은 422."""
...
def generate(
    ...
    """데이터를 ECharts option 으로 집계한다. 잘못된 입력은 422."""
