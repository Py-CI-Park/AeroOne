...
![version](https://img.shields.io/badge/version-1.6.1-1f6feb)
...
| 검증 | backend pytest + httpx (171 passed), frontend Vitest + Testing Library (188 passed, 47 파일), `tsc --noEmit` exit 0, `next build` 성공, Windows 실행 스모크 |
...
릴리스 1.6.1 기준 backend `pytest tests` 결과 **171 passed** (실패 0), frontend Vitest **188 passed** (47 파일), `tsc --noEmit` exit 0, `next build` 성공. 회귀 발생 시 [`docs/INDEX.md`](docs/INDEX.md) §7 테스트 인벤토리와 [`docs/reports/INDEX.md`](docs/reports/INDEX.md) 의 단계별 보고서를 거꾸로 읽어 어느 단계의 회귀인지 진단합니다.
