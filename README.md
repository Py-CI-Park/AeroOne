...
![version](https://img.shields.io/badge/version-1.16.0-1f6feb)
...
> [!CAUTION]
> **1.12.2 배포본은 철회되었습니다.** 현재 운영 반입물은 immutable 정식 `1.16.0`이며, 1.15.1/1.15.0/1.14.0/1.13.2/1.13.1/1.13.0은 변경하지 않는 역사 릴리스로 보존합니다(`1.15.0`은 오프라인 설치 차단 결함으로 `1.15.1`로 대체됨). `1.16.0`은 1.15.1 위에 대시보드 섹션 재편(AI/ETC), Civil Aircraft v1.7 인터랙티브 대시보드, Leantime 실기동 포터블 반입물(별도 ZIP), 버전 표기 정합을 얹은 forward-only feature 릴리스입니다. 이전 immutable 릴리스의 tag·asset·digest는 변경하지 않습니다. Release publication은 완료되어도 사람이 air-gapped network에 물리적으로 import했다는 의미는 아닙니다.
...
현재 최신 운영 반입물은 2026-07-14 게시된 immutable 정식 `1.16.0` Release의 `AeroOne-offline-1.16.0.zip`(+`.sha256`)이며, Leantime 실기동을 위한 **선택 반입물** `AeroOne-Leantime-Stack-v3.9.8-*.zip`(+`.sha256`)이 같은 Release에 함께 게시됩니다. Release API는 `immutable=true`를 반환했고, published 시각은 `2026-07-14T22:05:36Z`입니다. 이 릴리스는 no-ff merge commit `7ce3c30122dbde446b3f752d8d7c112fb6ded358`(1.15.1 merge `b57d13f` 위의 자식)에서 annotated tag object `a5c841883c10e20223c3cf97e39fc46917b775c1`로 게시되었습니다. 본체 ZIP size는 `235546946` bytes, SHA-256은 `b44d11b02327663c7af787a45197a25908c91545f182d6d8a87ca7d16b5ccf7f`이고, Leantime 스택 ZIP size는 `251460126` bytes, SHA-256은 `f6a30394c53eee082088311f1253d6f87d9daa02bcd55411b78c440cf4bd9fab`입니다. GitHub immutable releases 정책은 활성화되어 있으며, `1.15.1`(ZIP SHA-256 `1e2a3ebb10d9c4a57943604e810fef7432065ac7965df41f9f4a2cf1fd153f98`), `1.15.0`(`8fb46dc00f0ae16bebaebee44dd8bae64656e9527eda7b9fadf1c8bc90dcafd5`, 설치 차단으로 `1.15.1`로 대체), `1.14.0`(`f6fae644413b67449c6cefdf4a32e9d570416199a124b5c4de0eceadaa25a8f7`), `1.13.2`(`92d5178d6fb67573a1f0b36e0a744e00b4b559548081b463d45a4ba1d669d8a4`), `1.13.1`(`b05445b53ecca02175afcd016ac0e896163010e1a06a0b996b8ebe79a798e290`), `1.13.0`은 tag·asset·digest를 변경하지 않는 역사 릴리스로 그대로 보존합니다. Leantime 스택은 AGPL(Leantime)/GPL(MariaDB) 바이너리를 본체 ZIP과 분리 격리한 선택 반입물입니다. Release publication은 사람이 air-gapped network에 물리적으로 import했다는 뜻이 아니며, 로컬 중간 ZIP은 운영 배포본이 아닙니다.

| 파일 | 어디서 받는가 | 폐쇄망에서 놓을 위치 | 역할 |
|---|---|---|---|
| `AeroOne-offline-1.16.0.zip` + `.sha256` | [정식 immutable GitHub Release `1.16.0`](https://github.com/Py-CI-Park/AeroOne/releases/tag/1.16.0) asset (ZIP size `235546946` bytes, SHA-256 `b44d11b02327663c7af787a45197a25908c91545f182d6d8a87ca7d16b5ccf7f`) | `D:\AeroOne\` 로 압축 해제 | allow-list로 검증된 AeroOne 소스, production wheelhouse/node_modules, prebuilt `.next`, 정확한 Python 3.12.7 / Node 20.18.0 인스톨러 |
| `AeroOne-Leantime-Stack-v3.9.8-*.zip` + `.sha256` (선택) | 같은 `1.16.0` Release asset (ZIP size `251460126` bytes, SHA-256 `f6a30394c53eee082088311f1253d6f87d9daa02bcd55411b78c440cf4bd9fab`) | `D:\AeroOne-Leantime-Stack\` 로 압축 해제 | 포터블 PHP 8.3.32 + MariaDB 11.4.8 + Leantime v3.9.8 + setup/start/stop (폐쇄망 실기동, AGPL/GPL 격리) |
