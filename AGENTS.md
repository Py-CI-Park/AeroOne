...
현재 `1.18.0` 기준 backend **1,569 passed**(unit 1,040 + integration 529), frontend **653 passed / 101 files**를 유지합니다. 회귀 발생 시 [`docs/INDEX.md`](docs/INDEX.md) §7 표와 [`docs/reports/INDEX.md`](docs/reports/INDEX.md) 를 참고해 어느 단계의 회귀인지 진단합니다.
...
- backend `pytest tests` — 현재 기준 전체 건수(**1.18.0: 1,569 passed**) 유지 (실패 0). **부분 실행으로 병합 금지** — 1.16.3 이 28 failed 상태로 게시된 회귀의 재발 방지 게이트다. (환경상 credential rotation·batch subprocess 테스트가 간헐 hang 하면 leftover 프로세스를 정리하고 재실행; 1.18.0 은 unit/integration(회전 제외)/회전 3분할로 각각 green 확인 후 게시.)
- 스택/번들류 반입물(예: AeroOne-Leantime-Stack)은 게시 전 본 PC 에서 setup→start→로그인 E2E 1회 필수. "⚠ 운영자 검증 필요" 주석이 남은 반입물은 게시 차단.
...
- **성능 예산 게이트 (1.17.0 승격)** — 사용자 체감 3경로(대시보드 `/` · 뉴스레터 `/newsletters` · Civil `/reports/civil-aircraft`)의 라우트 First Load JS 상한을 `scripts/qa/release_budget_gate.mjs` 로 강제한다. 릴리스 직전 `frontend` 에서 `next build` 출력을 캡처한 뒤 `node scripts/qa/release_budget_gate.mjs --build-log <build.log> --version <X.Y.Z>` 를 실행해 실측 산출물(`artifacts/qa/release-budget/<X.Y.Z>.json`)을 남기고, 예산 초과(또는 경로 누락 fail-closed) 시 병합을 차단한다. 성능 점수·FCP 예산은 같은 매니페스트에 정의돼 있으며 Chrome 이 있는 본 PC 에서 Lighthouse 딥패스(`scripts/qa/run_v113_lighthouse.mjs`)로 측정한다. 자세한 절차: [`docs/runbook/release-performance-budget.md`](docs/runbook/release-performance-budget.md).
