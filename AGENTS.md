...
현재 `1.16.3` 기준 backend **1,179 passed**(unit 834 + integration 345), frontend **540 passed / 88 files**를 유지합니다. 회귀 발생 시 [`docs/INDEX.md`](docs/INDEX.md) §7 표와 [`docs/reports/INDEX.md`](docs/reports/INDEX.md) 를 참고해 어느 단계의 회귀인지 진단합니다.
...
- backend `pytest tests` — 현재 기준 전체 건수(**1.16.3 수리 후: 1,179 passed**) 유지 (실패 0). **부분 실행으로 병합 금지** — 1.16.3 이 28 failed 상태로 게시된 회귀의 재발 방지 게이트다.
