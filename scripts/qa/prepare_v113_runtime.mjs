...
const waitPort = () => new Promise((ok, bad) => { const s = createServer(); s.once('error', bad); s.listen(0, '127.0.0.1', () => { const p = s.address().port; s.close(() => ok(p)); }); });
...
let tempRoot; let backendPid; let frontendPid; const fds = [];
try {
 ...
 const start = (cmd,args,cwd,log) => { const fd=openSync(log,'a'); fds.push(fd); const p=spawn(cmd,args,{cwd,env,shell:false,windowsHide:true,detached:true,stdio:['ignore',fd,fd]}); p.unref(); return p.pid; }; backendPid=start(py,['-m','uvicorn','app.main:app','--host','127.0.0.1','--port',String(bp)],tempRoot,join(artifactRoot,'backend.log')); frontendPid=start(process.execPath,[next,'start','-H','127.0.0.1','-p',String(fp)],frontendCwd,join(artifactRoot,'frontend.log'));
 await writeFile(runtimePath,JSON.stringify({schemaVersion:1,sha,backendUrl:`http://127.0.0.1:${bp}`,frontendUrl:`http://127.0.0.1:${fp}`,backendPid,frontendPid,tempRoot,artifactRoot},null,2)+'\n',{mode:0o600}); const readyEnv=Object.fromEntries(allow.filter(k => process.env[k] !== undefined).map(k => [k, process.env[k]])); await run(process.execPath,[join(import.meta.dirname,'wait_v113_ready.mjs'),'--sha',sha],repo,readyEnv); process.stdout.write('runtime ready\n');
} catch (error) {
 const cleanupErrors = [];
