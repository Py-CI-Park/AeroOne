...
export function NewsletterDateCalendar({
  ...
  return (
    <section
      data-testid="newsletter-date-calendar-panel"
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">발행일을 선택하면 해당 뉴스레터 미리보기로 이동합니다.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 transition hover:border-slate-400 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
          >
            이전 달
          </button>
          <button
          ...
          <button
            ...
            className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-700 transition hover:border-blue-300 hover:bg-blue-100"
          >
            {open ? '달력 접기' : '달력 펼치기'}
          </button>
        </div>
      ...
      <div id="newsletter-calendar-grid" data-testid="newsletter-calendar-grid" hidden={!open} className="mt-4">
        ...
        <div className="grid grid-cols-7 gap-2">
          {cells.map((cell, index) => {
            if (!cell.day) {
              return <div key={`empty-${index}`} className="h-14 rounded-xl bg-slate-50" />;
            }
            if (!cell.entry) {
              return (
                <div
                  key={`inactive-${cell.day}`}
                  className="flex h-14 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white text-sm text-slate-300"
                >
                  {cell.day}
                </div>
              );
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] uppercase ${isSelected ? 'text-slate-200' : 'text-blue-500'}`}>
