 ...
 * 상위(LeantimeStatus)가 헬스 결과로 enabled 를 넘겨, 미구동이면 링크 대신 비활성 안내로
 * 렌더해 '눌러도 빈 화면'을 원천 차단한다.
 */
export function LeantimeLaunch({ port = 8081, enabled = true }: { port?: number; enabled?: boolean }) {
  ...
  if (!enabled) {
    return (
      <span
        aria-disabled
        title="Leantime 이 아직 설치·기동되지 않았습니다."
