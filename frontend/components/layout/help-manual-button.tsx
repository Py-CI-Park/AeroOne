...
const MANUAL_SECTIONS = [
  ...
  },
  {
    id: 'aero-work',
    label: 'Aero Work',
    title: 'Aero Work 업무 워크스페이스 (개발중)',
    rows: [
      ['업무대화', '대화 한 줄을 일정·문서·지식·할 일·도움말로 자동 라우팅합니다. 예: "내일 오전 10시 주간회의 일정 등록해줘"(일정), "예산 편성 근거 찾아줘"(지식 근거 답변), "내일까지 예산보고서 초안 할 일 추가해줘"(할 일). 파일(.pdf/.docx/.hwpx 등 최대 5개)을 첨부해 내용 근거로 물을 수 있고, 첨부 원문은 저장하지 않습니다.'],
      ['일정·할 일', '개인 캘린더에 일정을 추가·수정·삭제하고 주/월로 봅니다. 할 일은 마감·상태(할 일/진행/완료) 중심으로 관리합니다. 같은 제목 일정이 여럿이면 삭제 전 후보를 확인해 오삭제를 막습니다.'],
      ['문서작성(HWPX)', '제목·본문을 5개 양식(시행문/1페이지/풀버전/이메일/임의형식)으로 종이 미리보기한 뒤 한글 HWPX(OWPML)로 내려받습니다. 수정 지시를 입력하면 이전 초안을 반영해 다시 생성합니다.'],
      ['내 지식폴더', '내 계정 소유의 로컬 폴더를 그 자리에서 색인해 의미·키워드 검색과 근거 답변(실시간 스트리밍)을 씁니다. 다른 사용자의 폴더는 보이지 않습니다. 색인은 백그라운드로 진행률을 보며 다른 작업을 계속할 수 있습니다.'],
      ['사용법 탭·환경설정', 'Aero Work 안의 "사용법" 탭에서 기능별 활용 예시와 세팅법(로컬 AI 연결·OpenAI 호환·지식폴더 등록)을 봅니다. 환경설정에서 로컬 AI(Ollama/OpenAI 호환) 연결 상태와 LLM 프로필을 확인합니다.'],
    ],
  },
  {
    id: 'viewer',
