...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2026-07-17',
    items: [
      'Civil Aircraft 대시보드를 v1.8 로 개정했습니다 — 실척 실루엣에 마우스를 올리면 치수가 뜨고 클릭하면 백과 상세로 이동하며, 비교 도식을 PNG·SVG 로 내려받을 수 있습니다. 제조사·세대별 형상과 측면 후퇴각 표현을 정교화했고(사양값·출처는 불변, 통제도면 아님), 포털 첫 화면은 경량 메타만 불러와 더 빠르게 뜹니다.',
      '관리자 콘솔을 6개 그룹(개요·계정·콘텐츠·시스템·AI·감사)으로 재편했습니다 — 화면 진입 시 필요한 그룹의 데이터만 불러오도록 바꿔 첫 로딩 요청이 크게 줄었고, 기존 ?tab= 링크는 그대로 동작합니다.',
      '릴리스 성능 예산 게이트를 추가했습니다 — 대시보드·뉴스레터·Civil 3경로의 라우트 JS 크기 상한을 정의하고 릴리스마다 실측 기록을 남겨 성능 회귀를 막습니다.',
      '누적 부채를 정리했습니다 — 백엔드/프런트 테스트 경고를 0 으로 줄이고, 문서 목록 성능과 개발 초기 설치의 의존성 고정을 강화했습니다.',
    ],
  },
  {
    version: '1.17.1',
    date: '2026-07-17',
    items: [
      'Document/NSA 문서 목록에 파일 수정일이 표시됩니다 — 각 문서 오른쪽에 YYYY-MM-DD 로 보이고, 마우스를 올리면 이름과 함께 확인할 수 있습니다.',
      '문서 폴더 트리의 펼침 상태가 같은 탭 안에서 유지됩니다 — 문서를 오가거나 페이지를 새로 고쳐도 열어 둔 폴더가 다시 접히지 않습니다.',
      '뉴스레터 본문 아래에 이전/다음 이슈 이동 링크가 추가되어 달력을 거치지 않고 연속으로 읽을 수 있습니다.',
      'NSA 접근은 계정 권한(collections.nsa.read) 기준으로 동작합니다 — 권한이 없는 계정에는 관리자에게 요청하라는 안내가 표시됩니다(기존 비밀번호 가림막 제거는 1.16.x 에 반영).',
    ],
  },
  {
    version: '1.17.0',
    date: '2026-07-17',
    items: [
      'Office Studio 차트 입력을 대화형 컴포저로 재편했습니다 — 한 입력창에 목적 문장을 적거나 표(CSV) 데이터를 그대로 붙여넣고, 파일을 클립 버튼·드래그앤드롭으로 첨부하면 열 구성을 자동 인식해 칩으로 보여 줍니다. 생성 결과 아래 후속 명령("상위 5개만", "가로 막대로", "누적으로")으로 같은 데이터를 대화하듯 반복해서 다듬을 수 있습니다(LLM 없이도 규칙 기반으로 동작).',
      'AeroAI 답변이 실시간 스트리밍으로 표시됩니다 — 첫 문장부터 점진적으로 렌더되고 중지 버튼으로 끊을 수 있으며, 대화 저장은 답변 완결 시점에만 수행됩니다. .md/.txt/.csv 파일을 최대 5개(합계 20만 자) 첨부해 그 내용을 근거로 질문할 수 있습니다(첨부 원문은 저장되지 않습니다).',
      '대시보드 Notebook/OpenWebUI 카드에 상태 배지를 추가했습니다(대시보드 진입 시 확인) — 준비된 경우에만 열기 링크가 활성화되어 죽은 링크 클릭이 사라집니다. 대시보드 상단에 "최근 본 뉴스레터" 스트립이 표시됩니다(이 PC 기준 열람 기록).',
      '개발 PC 초기 설치의 frontend 의존성 복원을 lockfile 고정(npm ci)으로 일원화해 로컬에서만 게이트가 깨지는 드리프트를 차단했습니다.',
      '스트리밍 중 마크다운 제목이 도착하는 순간 화면이 멈추던 치명 결함(파서 무한 루프)을 실측으로 찾아 근절했습니다.',
    ],
  },
  {
    version: '1.16.3',
