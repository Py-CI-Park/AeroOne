...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      'AeroAI 답변이 실시간 스트리밍으로 표시됩니다 — 첫 문장부터 점진적으로 렌더되고 중지 버튼으로 끊을 수 있으며, 대화 저장은 답변 완결 시점에만 수행됩니다. .md/.txt/.csv 파일을 최대 5개(합계 20만 자) 첨부해 그 내용을 근거로 질문할 수 있습니다(첨부 원문은 저장되지 않습니다). 스트리밍 중 마크다운 제목이 도착할 때 화면이 멈추던 치명 결함(파서 무한 루프)을 실측으로 찾아 근절했습니다.',
      ...
      'Document/NSA 문서 목록에 파일 수정일이 표시되고(각 문서 오른쪽 YYYY-MM-DD), 폴더 트리의 펼침 상태가 같은 탭 안에서 유지됩니다. 뉴스레터 본문 아래에 이전/다음 이슈 이동 링크가 추가되어 달력을 거치지 않고 연속으로 읽을 수 있습니다. NSA 접근은 계정 권한(collections.nsa.read) 기준으로 동작합니다(권한 없는 계정에는 요청 안내).',
      ...
      '개발 PC 초기 설치의 frontend 의존성 복원을 lockfile 고정(npm ci)으로 일원화하고, 백엔드/프런트 테스트 경고를 0 으로 줄이는 등 누적 부채를 정리했습니다.',
    ],
  },
  {
    version: '1.16.3',
