...
test('전송 시 orchestrateAeroWork 인자에 attachments 가 포함되고(synthesize:true) 성공 후 칩이 비워진다', async () => {
  orchestrateAeroWork.mockResolvedValue({
    utterance: '문의',
    session_id: 1,
    results: [baseResult({ routed_by: 'rule' })],
  });
  ...
  // B2: 첨부가 있으면 스트림 대신 synthesize:true 비스트리밍 경로로 분기한다.
  expect(orchestrateAeroWork).toHaveBeenCalledWith('문의', 'csrf-test-token', null, {
    synthesize: true,
    attachments: [{ name: 'note.md', content: '첨부 본문' }],
  });
  // 첨부 요청은 스트리밍을 타지 않는다 — 별도 지식 답변 스트림 호출이 없다.
  expect(streamAeroWorkAnswer).not.toHaveBeenCalled();
...
test('routed_by 가 llm 이면 응답 카드에 AI 보조 분류 배지가 표시된다', async () => {
  orchestrateAeroWork.mockResolvedValue({
    ...
    session_id: 2,
    // B3: routed_by 는 결과 항목(OrchestrateResult)마다 온다 — 응답 최상위에는 없다.
    results: [baseResult({ kind: 'knowledge', summary: '지식 검색 결과입니다.', routed_by: 'llm' })],
  });
