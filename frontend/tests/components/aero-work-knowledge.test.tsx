...
import { ApiError, fetchKnowledgeFolders, reindexKnowledgeFolder } from '@/lib/api';
...
vi.mock('@/lib/api', async () => {
  ...
  return {
    ...
    reindexKnowledgeFolder: vi.fn(async () => ({ status: 'indexing' })),
...
// G004 FrontendProgress — 재색인 202/409 + 색인 중 진행률 표시 회귀 고정.
describe('KnowledgePanel — 202 백그라운드 색인 진행률', () => {
  afterEach(() => {
    vi.mocked(fetchKnowledgeFolders).mockReset();
    vi.mocked(reindexKnowledgeFolder).mockReset();
  });

  test('folder.status가 indexing이면 status_detail을 표시하고 재색인 버튼을 비활성화한다', async () => {
    vi.mocked(fetchKnowledgeFolders).mockResolvedValue({
      folders: [
        {
          id: 2,
          name: '색인 중 폴더',
    ...
    await waitFor(() => expect(screen.getByText('37/120 파일')).toBeInTheDocument());
    const reindexButton = screen.getByRole('button', { name: '색인 중…' });
    expect(reindexButton).toBeDisabled();
  });

  test('재색인이 409를 받으면 이미 진행 중이라는 한국어 안내를 표시한다', async () => {
    vi.mocked(fetchKnowledgeFolders).mockResolvedValue({
      folders: [
        {
          id: 3,
    ...
    vi.mocked(reindexKnowledgeFolder).mockRejectedValueOnce(new ApiError('이미 색인 진행 중입니다', 409));

    render(<KnowledgePanel />);

    await waitFor(() => expect(screen.getAllByText('재색인 대상 폴더').length).toBeGreaterThan(0));
    fireEvent.click(screen.getByRole('button', { name: '재색인' }));

    await waitFor(() => expect(screen.getByText('이미 색인이 진행 중임. 완료될 때까지 기다릴 것.')).toBeInTheDocument());
  });
});
