...
const fetchLatestNewsletterMock = vi.fn();
const fetchNewsletterCalendarMock = vi.fn();
const fetchNewsletterDetailMock = vi.fn();
const fetchNewslettersMock = vi.fn();
const getServerApiBaseMock = vi.fn(() => 'http://localhost:18437');
...
vi.mock('@/components/newsletter/newsletter-recent-list', () => ({
  NewsletterRecentList: ({ items }: { items: NewsletterItem[] }) => <div data-testid="newsletter-recent-list">{items.length}</div>,
}));
...
test('renders newsletters content without the removed hero banner', async () => {
