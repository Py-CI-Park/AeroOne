...
test('operator dashboard groups cards into ordered sections and keeps coming-soon in Development', async () => {
  ...
  expect(screen.getByText('12 active · 2 coming soon')).toBeInTheDocument();
});

test('operator dashboard shows the three office-tools cards in Development', async () => {
  isAdminMock.mockReturnValue(true);
  render(await HomePage({ searchParams: Promise.resolve({}) }));

  const main = screen.getByRole('main');
  const reportLink = within(main).getByRole('link', { name: /보고서 스튜디오/ });
  const chartLink = within(main).getByRole('link', { name: /차트 스튜디오/ });
  const diagramLink = within(main).getByRole('link', { name: /다이어그램 스튜디오/ });

  expect(reportLink).toHaveAttribute('href', '/office-tools/report');
  expect(chartLink).toHaveAttribute('href', '/office-tools/chart');
  expect(diagramLink).toHaveAttribute('href', '/office-tools/diagram');
});

test('non-admin dashboard hides the admin-only office-tools cards', async () => {
  isAdminMock.mockReturnValue(false);
  render(await HomePage({ searchParams: Promise.resolve({}) }));

  expect(screen.queryByRole('link', { name: /보고서 스튜디오/ })).not.toBeInTheDocument();
  expect(screen.queryByRole('link', { name: /차트 스튜디오/ })).not.toBeInTheDocument();
  expect(screen.queryByRole('link', { name: /다이어그램 스튜디오/ })).not.toBeInTheDocument();
});
