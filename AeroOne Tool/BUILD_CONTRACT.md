# AeroOne 통합 빌드 계약서 — 역사적 구현 제안 스냅샷 (2026-07-11)

> **문서 상태 — 역사 기록:** 이 문서의 §0~§8은 2026-07-11 당시의 구현 제안과 작업 계약을
> 보존한다. 현재 코드·운영 계약이 아니며, 아래의 경로·마이그레이션·카드 수·권한·테스트 게이트를
> 실행 지시나 현재 사실로 해석해서는 안 된다.
>
> **2026-07-13 현재 진실 원천**
>
> - Office Studio 운영 계약: [`docs/runbook/office-tools.md`](../docs/runbook/office-tools.md) —
>   `office.use`, 단일 허브(`/office-tools`), 규칙 기반 폴백, 413/422 입력 계약, owner 격리와
>   lifecycle 제어.
> - 카드 최종 상태: `backend/alembic/versions/20260712_0012_office_tools_single_card.py`,
>   `20260712_0013_office_tools_english_title.py`, `20260712_0014_leantime_internal_landing.py`,
>   `backend/app/modules/admin/api.py`, `frontend/app/page.tsx`. Alembic head는
>   `20260712_0014`; fallback ID는 Office Studio `11`, Leantime `12`다.
> - Leantime 동거 운영 계약: [`docs/runbook/leantime-codeploy.md`](../docs/runbook/leantime-codeploy.md).
>   카드는 외부 `8081` 링크가 아니라 내부 안내 페이지 `/leantime`으로 이동한다.
> - 구현 시점 평가와 남은 Leantime 준비도 조건:
>   [`docs/runbook/office-leantime-architecture-review-2026-07-13.md`](../docs/runbook/office-leantime-architecture-review-2026-07-13.md).
>
> 아래의 당시 파일 목록, `0008~0010` 체인, 개별 Office 카드, 외부 Leantime 링크 및 최종 게이트는
> 역사적 제안의 일부다. 현재 런북이나 소스보다 우선하지 않는다.


---

## 0. 2026-07-11 당시 제안 산출물 (역사 기록)
...
3. **암호화 키가 `jwt_secret_key` 파생**: 시크릿 회전 시 기존 `api_key_encrypted` 복호 불가 → 재등록 필요. 런북에 명시. development에서도 배포별 고유 시크릿을 사용해야 하며, production/closed_network 은 config 가 ≥32자 강제.
